...
async def run_with_router_and_train(
    ...
    log_dir = Path(f"{AgentPrune_ROOT}/result/Router-NodeMerge_MultiArith_{current_time}/logs")
    ...
    # 5. 평가 (MultiArith Evaluation) - T_inference 단계
...
async def run_multiarith_experiment(
    ...
    print(f"\nAccuracy: {result['evaluation']['accuracy']*100:.1f}% ({result['evaluation']['total_solved']}/{result['evaluation']['total_executed']})")
