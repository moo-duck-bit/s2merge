...
def agent_conversation_log(
    ...
    """
    Log agent conversation without affecting token count.

    This function logs agent conversations to a separate file without
    triggering token counting mechanisms.

    Args:
        agent_name (str): Name/role of the agent
        round_num (int): Current round number
        input_text (str): Input prompt/message to the agent
        output_text (str): Output/response from the agent
        log_file_path (Path): Path to the conversation log file
    """
    formatted_message = (
        f"\n{'='*80}\n"
    ...
    try:
        ...
        logger.error(f"Error writing agent conversation log: {error}")
