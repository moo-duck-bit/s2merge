...
class Graph(ABC):
    ...
    def __init__(self, 
        ...
        self.dec_1=False
        self.skip_nodes = []
        self.conversation_log_path = None  # Will be set externally if logging is needed
    ...
    async def arun(self, input: Dict[str,str], 
            ...
            while zero_in_degree_queue:
                ...
                while tries < max_tries:
                    try:
                        ...
                        # Log agent conversation if logging is enabled
                        if self.conversation_log_path is not None:
                            try:
                                from AgentDropout.utils.log import agent_conversation_log
                                current_node = self.nodes[current_node_id]
                                input_text = f"Task: {input.get('task', 'N/A')}"
                                # Handle None outputs safely
                                if current_node.outputs:
                                    if isinstance(current_node.outputs, list):
                                        output_text = '\n'.join([str(o) if o is not None else 'None' for o in current_node.outputs])
                                    else:
                                        output_text = str(current_node.outputs) if current_node.outputs is not None else 'None'
                                else:
                                    output_text = 'No output'
                                agent_conversation_log(
                                    agent_name=f"{current_node.role} (ID: {current_node_id})",
                                    round_num=round + 1,
                                ...
                                print(f"Warning: Failed to log conversation for node {current_node_id}: {e}")
        ...
        if len(self.potential_spatial_edges)>0:
            ...
            # Log decision node conversation if logging is enabled
            if self.conversation_log_path is not None:
                try:
                    from AgentDropout.utils.log import agent_conversation_log
                    input_text = f"Task: {input.get('task', 'N/A')}\nAll agent outputs aggregated"
                    output_text = '\n'.join(final_answers) if isinstance(final_answers, list) else str(final_answers)
                    agent_conversation_log(
                        agent_name=f"{self.decision_node.role} (Decision Node)",
                    ...
                    print(f"Warning: Failed to log decision node conversation: {e}")
        else:
