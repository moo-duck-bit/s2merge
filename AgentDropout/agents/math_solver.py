...
class MathSolver(Node):
    ...
    def _process_inputs(self, raw_inputs:Dict[str,str], spatial_info:Dict[str,Dict], temporal_info:Dict[str,Dict], **kwargs)->List[Any]:
        ...
        # Limit context length to prevent exceeding model's max tokens
        MAX_INFO_LENGTH = 2000  # characters per info to stay within 8192 tokens
        if self.role == "Math Solver":
            ...
            for id, info in spatial_info.items():
                # Skip None outputs and truncate long outputs
                output = info.get('output', '')
                if output and output != 'None.':
                    # Truncate if too long
                    if len(str(output)) > MAX_INFO_LENGTH:
                        output = str(output)[:MAX_INFO_LENGTH] + "... (truncated)"
